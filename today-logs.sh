...
if [ $? -ne 0 ]
then
    echo -e "$G mysql is not installed. lets do it. $N" &>>$LOG_FILE
    dnf install mysql -y
    VALIDATE $? "mysql"
else
...
if [ $? -ne 0 ]
then
    echo -e "$G python3 is not installed. lets do it. $N" &>>$LOG_FILE
...
if [ $? -ne 0 ]
then
    echo -e "$G nginx is not installed. lets do it. $N" &>>$LOG_FILE
